...
const JavaScript = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setJAVASCRIPT(data[2].JAVASCRIPT);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  const getDisplayContent = () => {
    ...
    if (selectedContent && selectedTopic && selectedSub && selectedMain) {
      const content =
      JAVASCRIPT[selectedMain][selectedSub][selectedTopic][selectedContent];
      return {
    ...
    if (selectedTopic && selectedSub && selectedMain) {
      const topic = JAVASCRIPT[selectedMain][selectedSub][selectedTopic];
    ...
    if (selectedSub && selectedMain) {
      const sub = JAVASCRIPT[selectedMain][selectedSub];
    ...
    if (selectedMain) {
      const main = JAVASCRIPT[selectedMain];
  ...
  return (
    ...
    >
      ...
      >
        <h1 className="text-xl font-bold mb-4">JAVASCRIPT Tutorial</h1>
        {Object.entries(JAVASCRIPT).map(([main, mainContent]) => (
        ...
        {displayContent ? (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
              ...
              <span style={{ color: "#C586C0", fontSize: "14px" }}>JAVASCRIPT</span>
            </div>
